<template>
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-input placeholder="请输入队伍ID" v-model="teamid" clearable @clear="getList">
            <el-button slot="append" icon="el-icon-search" @click="selectByTeamId"></el-button>
          </el-input>
        </el-col>
        <el-col :span="40">
      ...
      <el-table :data="list" border stripe  max-height="470">
        ...
        <el-table-column prop="FirePlace" label="起火地点"></el-table-column>
        <el-table-column prop="LongitudeCoordinates" label="经度坐标"></el-table-column>
        <el-table-column prop="LatitudeCoordinates" label="纬度坐标"></el-table-column>

        <el-table-column label="事件时间">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="look1(scope.row)">查看</el-button>
          </template>
        </el-table-column>

        <el-table-column prop="experiment" label="起火原因">
          <template slot-scope="scope">
            <el-popover placement="bottom" width="200" trigger="click" v-bind:content="scope.row.FireCause">
              <el-button type="primary" size="mini" slot="reference">查看</el-button>
            </el-popover>
          </template>
        </el-table-column>

        <el-table-column prop="FireLevel" label="火灾等级"></el-table-column>
        <el-table-column prop="BurnedArea" label="过火面积"></el-table-column>
        <el-table-column prop="experiment" label="出动人员">
          <template slot-scope="scope">
            <el-popover placement="bottom" width="200" trigger="click" v-bind:content="scope.row.FireProtectionTeam">
              <el-button type="primary" size="mini" slot="reference">查看</el-button>
            </el-popover>
          </template>
        </el-table-column>

        <el-table-column prop="experiment" label="发现方式">
          <template slot-scope="scope">
            <el-popover placement="bottom" width="200" trigger="click" v-bind:content="scope.row.DiscoveredManner">
              <el-button type="primary" size="mini" slot="reference">查看</el-button>
            </el-popover>
          </template>
        </el-table-column>

        <el-table-column fixed="right" label="操作" width="90px" header-align="center" align="center">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" circle @click="showEditDialog(scope.row)">
            </el-button>
            <el-button type="danger" icon="el-icon-delete" size="mini" circle @click="removeById(scope.row)"></el-button>
          </template>
        </el-table-column>
      </el-table>

      <el-dialog title="事件时间" width="50%" :visible.sync="dialogTableVisible1">
        <el-table :data="timeList">
          <el-table-column type="index" label="#"></el-table-column>
          <el-table-column prop="DiscoveredTime" label="发现时间"></el-table-column>
          <el-table-column prop="ExtinguishingTime" label="灭火时间"></el-table-column>
          <el-table-column prop="RescueTime" label="扑救时间"></el-table-column>
        </el-table>
      </el-dialog>
...
export default {
  data() {
    return {
      ...
      list: [
        {
          'FirePlace':'位置1',
          'LongitudeCoordinates':'119.507244',
          'LatitudeCoordinates':'27.656711',
          'DiscoveredTime':'2020-03-05',
          'ExtinguishingTime':'2020-03-05',
          'RescueTime':'2020-03-05',
          'FireCause':'自然起火',
          'FireLevel':'二级',
          'BurnedArea':'**',
          'FireProtectionTeam':'**',
  ...
  methods: {
    ...
    //查单个
    async selectByTeamId() {
      if (this.teamid.length < 1) return this.$message.error('请输入ID')
      const {
        data: res
      } = await this.$http.get('firePro/selectTeam/' + this.teamid)
      if (res.code !== 200) {
